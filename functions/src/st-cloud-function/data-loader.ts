...
export async function loadSymbolBars(
  ...
  logger.info('st_data_loader_fetching', { runId, symbol, marketDate });
  const { dailyBars, weeklyBars, monthlyBars } = await getCachedBarsFromSymbolData(symbol, marketDate);
  logger.info('st_data_loader_loaded', {
    runId,
    symbol,
...
export async function getCachedBarsFromSymbolData(
  ...
  try {
    ...
    logger.info('st_symbol_data_loader_query', {
      symbol,
      marketDate,
    ...
    if (yearShards.empty) {
      logger.warn('st_symbol_data_loader_cache_miss', { symbol, marketDate });
    ...
    logger.info('st_symbol_data_loader_result', {
      symbol,
      marketDate,
      dailyBars: dailyBars.length,
      weeklyBars: weeklyBars.length,
    ...
    logger.error('st_symbol_data_loader_error', { symbol, marketDate, error: error?.message });
    return { dailyBars: [], weeklyBars: [], monthlyBars: [] };
  }
}
