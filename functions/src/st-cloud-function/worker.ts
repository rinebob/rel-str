...
import { loadSymbolBars } from './data-loader';
import { syncSymbolToSymbolData } from '../symbol-data-sync/symbol-data-backfill';
import { persistSymbolSignals } from './signal-persister';
import { RunProgressTracker } from './run-progress';

import { SymbolJobPayload } from '../common/st-shared-types';
 ...
 */
export const stProcessSymbol = onTaskDispatched<SymbolJobPayload>(
  {
    retryConfig: {
      maxAttempts: 3,
      minBackoffSeconds: 5,
  ...
  async (req) => {
    ...
    logger.info('st_worker_start', { runId, symbol, marketDate });
    ...
    try {
      ...
      if (!sufficient) {
        logger.warn('st_worker_insufficient_data', { runId, symbol, barCount: dailyBars.length, required: MIN_REQUIRED_BARS });
      ...
      if (dailyBars.length < strategy.metadata.minBarsRequired) {
        logger.warn('st_worker_insufficient_bars_for_strategy', {
      ...
      logger.info('st_worker_complete', {
        runId,
        symbol,
        durationMs: duration,
...
async function executeStrategy(
  ...
  logger.info('st_worker_executing_strategy', {
  ...
  if (!validation.valid) {
    ...
    logger.error('st_worker_strategy_config_invalid', { runId, symbol: input.symbol, strategy: strategyName, errors: validation.errors });
  ...
  logger.info('st_worker_strategy_result', {
    runId, symbol: input.symbol, strategy: strategyName,
