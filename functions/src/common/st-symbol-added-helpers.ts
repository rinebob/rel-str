...
import { StSymbolSource } from './st-collections';
 ...
 */
export function normalizeSource(payloadSource: string | undefined): StSymbolSource {
  if (payloadSource === StSymbolSource.PARTNER_UNIVERSE) {
    return StSymbolSource.PARTNER_UNIVERSE;
  }
  if (payloadSource === StSymbolSource.MANUAL_ADD) {
    return StSymbolSource.MANUAL_ADD;
  }
  return StSymbolSource.MANUAL_ADD;
}
