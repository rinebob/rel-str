...
* **Communication:** Firebase SDKs (Authentication, Firestore) and calls to Firebase Cloud Functions
    * *Approach:* The frontend will interact with the backend using Firebase SDKs for direct interactions (e.g., Authentication, listening to Firestore data changes) and Angular's `HttpClient` (via NgRx signal store and dedicated service) for calling Firebase Cloud Functions that read from Firestore. The Angular app will not call partner endpoints directly; partner endpoints are server-to-server only.
