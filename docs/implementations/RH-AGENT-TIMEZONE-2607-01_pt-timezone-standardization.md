...
Implementation in progress — Phases 1-4 complete. Phase 5 (documentation and verification) and optional Phase 6 (schedule configuration) remaining.
...
| `functions/src/rh-agent-cloud-function/rh-agent-shared.ts` | `getMarketDate()` at line 28 | PT date | Correct, but not used by symbol-data-sync |
| `functions/src/rh-agent-cloud-function/rh-agent-shared.ts` | `generateRunId()` at line 73 | Uses PT time for suffix, PT date from caller | Correct for RH Agent runs, but `marketDate` passed in may be UTC |
...
- **Backend:** `getMarketDate()` in `functions/src/rh-agent-cloud-function/rh-agent-shared.ts` already returns `YYYY-MM-DD` in PT.
...
Create a dedicated `functions/src/rh-agent-cloud-function/rh-agent-date-utils.ts` (or extend `rh-agent-shared.ts`) with:

```typescript
export function getMarketDatePT(): string;
export function getRunDatePT(): string;
export function getRunIdPT(runDate: string, trigger: RhAgentTriggeredBy): string;
export function formatTimestampPT(ts: Date | FirebaseFirestore.Timestamp | string): string;
export function normalizeMarketDate(dateStr: string): string;
...
- `trigger` is one of `pdr`, `nightly`, `manual`.
...
- Rename/extend `getMarketDate()` to clearly mean the **trading date** (`marketDate`), or keep the name and add a separate `getRunDatePT()` helper.
- Add `getRunDatePT()` as the canonical helper for the **run occurrence date**.
- Update `generateRunId()` to use `getRunIdPT(runDate, trigger)` and include the trigger suffix.
- Remove any duplicated date-formatting logic.
